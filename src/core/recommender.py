def generate_recommendations(student_data: dict) -> list:
    ...
    if forum_count < 2 and forum_score > quiz_avg:
        recs.append("💬 Участие в форуме повышает вашу оценку. Попробуйте писать ≥2 поста в неделю.")

    if act.get("quiz_attempt", 0) > 3:
        recs.append("🔁 Повторные попытки тестов снижают эффективность. Лучше подготовиться заранее.")

    if act.get("view", 0) == 0 and act.get("video", 0) > 0:
        recs.append("📄 Лекции в текстовом формате помогают глубже понять материал — попробуйте их.")
